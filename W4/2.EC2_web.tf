...
resource "aws_instance" "web" {
  ...
  tags = {
    Name = format(
      "%s-%s-%s-web-%s",
      var.title,var.environments,var.subtitle,element(split("", aws_subnet.vpc_private_web[count.index].availability_zone), length(aws_subnet.vpc_private_web[count.index].availability_zone) - 1)
    )
  }  
}
